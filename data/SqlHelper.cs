...
public class SqlHelper
{
    ...
    public static readonly string connectionStringreg = ConfigurationManager.ConnectionStrings["connectionstringreg"].ConnectionString;
    ...
    public static DataSet RegExecuteDataSet(string cmdtext)
    {
        using (SqlConnection con = new SqlConnection(connectionStringreg))
        {
            SqlDataAdapter adapter = new SqlDataAdapter();
            using (SqlCommand cmd = new SqlCommand())
            {
...
END
    ...
    /// <param name="tableid">操作的ID</param>
    /// <param name="logtype">日志的类型</param>
    /// <returns></returns>
    public static int AddLog(string uid,string tablename,string tableid,int logtype) {
        string sql = "insert into LogTable values('"+uid+"','"+tablename+"','"+tableid+"','"+logtype+"','"+DateTime.Now+"')";
